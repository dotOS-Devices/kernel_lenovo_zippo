...
int goodix_ts_power_on(struct goodix_ts_core *core_data)
{
	...
	if (r < 0) {
		ts_info("%s: Failed to set regulator current avdd\n",
			__func__);
		return r;
	}
...
static int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	...
	if (core_data->irq_wake) {
		disable_irq_wake(core_data->irq);
		core_data->irq_wake = 0;
	}
...
#ifdef CONFIG_FB
static void goodix_ts_queued_resume(struct work_struct *w)
{
	struct goodix_ts_core *rmi4_data =
		container_of(w, struct goodix_ts_core, resume_work);
	goodix_ts_resume(rmi4_data);
}
	...
	if (evdata && evdata->data && core_data) {
		if (event == MSM_DRM_EVENT_BLANK) {
			...
			if (blank == MSM_DRM_BLANK_UNBLANK)
				queue_work(system_wq, &core_data->resume_work);
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	ts_device->hw_ops->avdd_en(ts_device);
	...
	INIT_WORK(&core_data->resume_work, goodix_ts_queued_resume);
out:
